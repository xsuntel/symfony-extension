...
- `activationEvents` includes a workspace-scoped event (`workspaceContains:**/symfony.lock` or `workspaceContains:**/bin/console`) — `onLanguage:php` alone activates the extension in every PHP project, not just Symfony ones.
...
- New provider logic or changes to `console.js` include at least one test in `test/extension.test.js`.
