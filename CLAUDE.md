...
│   ├── agent-memory/app/                ← Per-agent memory: base/ (typescript-code-* analyzer/reviewer/debugger/tester) + agent/multi-team
│   ├── agents/
│   │   ├── app/
│   │   │   ├── agent/                     ← multi-team (multi-step orchestrator over the app/base specialists)
│   │   │   └── base/                     ← typescript-code-* (analyzer, reviewer, debugger, tester)
│   │   └── utility/                      ← claude/ · git/ (each: code/commit author + reviewer)
│   ├── commands/                         ← nested paths OK; slug becomes the command name
│   │   ├── app/base/                     ← typescript-code-review
│   │   └── utility/claude/               ← code-config-review
│   ├── docs/
│   │   ├── app/
│   │   │   ├── agent/                     ← multi-docs (agent roster & orchestration reference)
│   │   │   └── base/                     ← typescript-code-docs (extension source reference)
...
│   ├── rules/                            ← structure is immutable: no moves, renames, or flattening
│   │   ├── app/base/                     ← reserved (empty)
│   │   ├── tools/
│   │   │   └── vscode-extension-rule.md  ← VSCode API rules & quick-reference for Claude
│   │   └── utility/claude/
│   │       └── config-structure-rule.md  ← .claude/ layout immutability (SoT)
│   ├── scripts/statusline.sh             ← Status line renderer (vendored)
│   ├── skills/                           ← flat: one dir per skill (nested dirs are not discovered)
│   │   ├── typescript-code-helper/       ← review-role dispatcher
│   │   ├── cc-config-helper/             ← .claude config author→reviewer team
│   │   ├── git-commit-helper/            ← commit message author→reviewer team
│   │   └── vscode-extension-helper/      ← inline VSCode API / structure guidance
...
- [.claude/rules/utility/claude/config-structure-rule.md](.claude/rules/utility/claude/config-structure-rule.md) — `.claude/` structure immutability: moving, renaming, or flattening its directories is prohibited
...
| debugging | `typescript-code-debugger` — root-cause tracing (read-only) |
| test authoring | `typescript-code-tester` |
| `security-auditor` (before deploy) | no dedicated agent — use the `/security-review` skill |
| _(no global role)_ | `typescript-code-analyzer` — proactive structural / code-health analysis (read-only) |
| _(no global role)_ | `multi-team` — multi-step orchestrator coordinating the `app/base` specialists for compound tasks |
