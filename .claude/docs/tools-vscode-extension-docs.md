# VSCode Extension Reference (TypeScript)
...
> It is the shared reference companion for the whole `tools-vscode-extension-` domain — the
> [`tools-vscode-extension-config-skill`](../skills/tools-vscode-extension-config-skill/SKILL.md),
> the [`tools-vscode-extension-scaffold-skill`](../skills/tools-vscode-extension-scaffold-skill/SKILL.md)
> author/reviewer pair, and the
> [`tools-vscode-extension-rule.md`](../rules/tools-vscode-extension-rule.md) SoT.
...
| `activationEvents` | `["onLanguage:php", "onLanguage:yaml", "workspaceContains:**/bin/console"]` | The `workspaceContains` guard already scopes activation to Symfony workspaces |
| `categories` | `["Programming Languages", "Other"]` | Marketplace grouping |
...
{
  "compilerOptions": {
    "module": "commonjs",         // CommonJS emit the Extension Host require()s
    "target": "ES2022",
    "outDir": "out",
    "rootDir": "src",
...
| `workspaceContains:**/bin/console` | Confirms a Symfony workspace — **already declared** in the manifest |
...
(`execSync` to `bin/console`) behind lazy calls, not the activation path.
