---
name: tools-vscode-extension-config-skill
...
# VSCode Extension Config Skill

A unified skill covering codebase analysis, VSCode Extension API guidance,
...
```text
package.json (activationEvents: onLanguage:php | onLanguage:yaml | workspaceContains:**/bin/console)
...
getProjectRoot(): workspace folder containing bin/console → null if none
  └── run(args): execSync('php bin/console <args>', timeout 10s)
...
single canonical definition from the `app-typescript-code-reviewer` agent
(`.claude/agents/app-typescript-code-reviewer.md`):

- **Checklist** — Correctness, Module System & Style, VSCode API Discipline,
  Security, Performance sections. Its **Type Safety** section defers in turn to
  `.claude/output-styles/app-typescript-code-style.md` §Type Safety, which is the
  SoT for the typing criteria themselves; the agent supplies only the severity
  mapping.
...
`/app-typescript-code-review` command
(`.claude/commands/app-typescript-code-review.md`) instead.
...
| Config reference (manifest, tsconfig, build, activation, contributions) | `.claude/docs/tools-vscode-extension-docs.md` |
| Output presentation (manifest fragments, citation, verdict format) | `.claude/output-styles/tools-vscode-extension-style.md` |
| Source code reference (TypeScript: entry, data layer, providers, views) | `.claude/docs/app-typescript-code-docs.md` |
| Extension architecture | `app/CLAUDE.md`, `app/src/CLAUDE.md` |
| TypeScript style (tsc → out, formatting, type safety) | `.claude/output-styles/app-typescript-code-style.md` |
