...
- API 동작 → 문서 검색(WebFetch) 결과 기반으로만 주장
...
### TypeScript / VSCode Extension 출력 예시

> 전체 규칙: `.claude/output-styles/app-typescript-code-style.md`
> API 퀵 레퍼런스: `.claude/rules/tools-vscode-extension-rule.md`

**핵심 규칙:**

- TypeScript 소스(`src/**/*.ts`, ES module `import` / `export`)를 `tsc -p ./`로 `out/`에 컴파일 — `src`에 `require` / `module.exports`를 직접 작성하지 않음
- `strict` 모드가 1차 정합성 도구: 정당한 사유 없는 `any` / `@ts-ignore` 금지, 캐스팅 대신 `unknown` 내로잉
- `var` 금지, `const` 기본; 세미콜론 필수, 단따옴표, 4 space indent
- 모든 disposable(프로바이더, 커맨드, 트리 뷰)을 `context.subscriptions`에 push
- 프로바이더는 `vscode.ProviderResult<T>`를 반환하고 실패 시 우아하게 축소(`null` / `[]` / `{}`) — throw 금지
- export 멤버·프로바이더 메서드는 명시적 반환 타입 선언
...
export class SymfonyHoverProvider implements vscode.HoverProvider {
    provideHover(
        ...
        const service = symfonyConsole.getServices()[match[1]];
        if (!service) {
            return null;
        }
