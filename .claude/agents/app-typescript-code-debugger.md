---
name: app-typescript-code-debugger
description: VSCode extension work — the extension.ts entry point, language providers (completion/hover/definition), tree views, the src/symfony/console.ts data layer, and the package.json manifest. Activate to diagnose a reported TypeScript extension runtime symptom (extension not activating, stale compiled output, completions not firing, empty tree views, command not found, stale data, go-to-definition misses) and trace it to a single root cause with a minimal fix. For proactive whole-codebase structure/health analysis use app-typescript-code-analyzer; for diff quality review use app-typescript-code-reviewer.
model: opus
maxTurns: 30
...
package.json
  ├── activationEvents: onLanguage:php | onLanguage:yaml | workspaceContains:**/bin/console   ← nothing runs before one fires
...
src/symfony/console.ts (singleton, all data flows through it)
  └── getProjectRoot(): string | null        — first workspace folder containing bin/console
        └── run(args: string, projectRoot: string): unknown — execSync('php bin/console <args>', timeout 10s)
              → JSON.parse (typed unknown) → narrow → 30s TTL cache → null on any failure
                → getServices()/getRoutes()/getParameters() return {} when null
```
...
| Completions fire but list is empty | `getProjectRoot()` returned `null` (no `bin/console` in any workspace folder) · `php` not on PATH · `bin/console` exited non-zero → `run` returns `null` → `{}` | `app/src/symfony/console.ts` — run the command manually in the fixture project |
