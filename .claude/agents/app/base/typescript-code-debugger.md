---
name: typescript-code-debugger
description: VSCode extension work — the extension.ts entry point, language providers (completion/hover/definition), tree views, the src/symfony/console.ts data layer, and the package.json manifest. Activate to diagnose a reported TypeScript extension runtime symptom (extension not activating, stale compiled output, completions not firing, empty tree views, command not found, stale data, go-to-definition misses) and trace it to a single root cause with a minimal fix. For proactive whole-codebase structure/health analysis use typescript-code-analyzer; for diff quality review use typescript-code-reviewer.
model: opus
memory: project
isolation: worktree
maxTurns: 30
