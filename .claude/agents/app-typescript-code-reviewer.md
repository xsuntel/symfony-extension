---
name: app-typescript-code-reviewer
...
- **Manifest**: `app/package.json` — `main: ./out/extension.js`, `scripts.compile` (`tsc -p ./`) / `scripts.watch`, `engines.vscode ^1.120.0`, `activationEvents: onLanguage:php|yaml + workspaceContains:**/bin/console`, `contributes` (views container `symfony`, three views, three commands: `symfony.refresh` / `symfony.filter` / `symfony.clearFilter`).
...
- **Notes**: `.claude/agent-memory/app-typescript-code-reviewer/MEMORY.md` records recurring issues and conventions confirmed in this codebase. Your `MEMORY.md` is injected into your system prompt at startup by `memory: project` — it is already in context, so do not spend a Read call retrieving it. The notes are hand-maintained and you cannot modify them (`Write` / `Edit` are disallowed for this agent); if one is wrong, say so in your report. The live source is the final authority when they conflict.
...
The type-safety criteria are the SoT in
[`.claude/output-styles/app-typescript-code-style.md`](../output-styles/app-typescript-code-style.md)
§Type Safety (plus §Module System for `import type`, and §Functions & Async for explicit return
types). Apply that section as written — this agent does not restate it. What is specific to a
review is how each violation is graded:

| Violation | Severity |
| --- | --- |
| `any` (implicit or explicit), unsafe assertion (`as T`, `as unknown as T`, non-null `!`), or anything failing `tsc --noEmit` | `[MUST]` |
| `JSON.parse` result used without `unknown` + a type guard; a Symfony shape cast rather than narrowed | `[MUST]` |
| `@ts-ignore` / `@ts-expect-error` without a justifying comment | `[MUST]` |
| Missing explicit return type on a public function, provider method, or exported member | `[SHOULD]` |
| `strictNullChecks` respected but the optional value guarded awkwardly; runtime `import` kept alive solely for a type | `[SHOULD]` |
| A stricter type that would remove a guard entirely | `[CONSIDER]` |
...
### Module System & Style (see `.claude/output-styles/app-typescript-code-style.md` for shared formatting)
...
- `app/src/symfony/console.ts:NN` — folding the `getRoutes()` array-vs-object handling into a single typed normaliser would remove the repeated shape check (stricter-typing opportunity).
...
| Shared formatting (semicolons, quotes, indent) | `.claude/output-styles/app-typescript-code-style.md` |
| Extension architecture | `app/CLAUDE.md`, `app/src/CLAUDE.md` |
| Per-file deep review | `/app-typescript-code-review` command |
