...
Apply the type-safety criteria from
[`.claude/output-styles/app-typescript-code-style.md`](../output-styles/app-typescript-code-style.md)
§Type Safety — `any`, `JSON.parse` → `unknown` + narrowing, unsafe assertions, domain interfaces
for Symfony shapes, and `strictNullChecks`. That section is the SoT; this command does not restate
it. Flag every violation of it with the exact line number.

A per-file deep audit additionally checks the following, which the SoT does not cover because they
matter only when reading one file end to end:

- **`noImplicitAny` disabled**: flag it if `app/tsconfig.json` has turned the check off — the SoT assumes strict mode is on.
- **`@ts-expect-error` preferred over `@ts-ignore`**: both need a justifying comment per the SoT; at audit depth also prefer `@ts-expect-error`, which fails once the underlying error disappears.
...
- **Repeated inline shapes**: flag the same structural object type written out more than once *within this file* — extract it to a named `interface` / `type`.
