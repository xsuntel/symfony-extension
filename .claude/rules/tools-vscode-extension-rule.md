...
- **Non-Symfony activation**: `onLanguage:php` alone fires for any PHP project, so pair it with a `workspaceContains:**/bin/console` (or `**/symfony.lock`) guard. This project already declares that guard — do not raise it as a missing one.
