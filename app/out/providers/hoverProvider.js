...
class SymfonyHoverProvider {
    ...
    async serviceHover(id, token) {
        const def = (await console_1.symfonyConsole.getServices())[id];
    ...
    async routeHover(name, token) {
        const def = (await console_1.symfonyConsole.getRoutes())[name];
        if (!def || token.isCancellationRequested) {
            return null;
        }
        const md = new vscode.MarkdownString();
    ...
    async paramHover(name, token) {
        const params = await console_1.symfonyConsole.getParameters();
