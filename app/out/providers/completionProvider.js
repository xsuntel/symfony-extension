...
class SymfonyCompletionItem extends vscode.CompletionItem {
    describe;
    constructor(label, kind, describe) {
        super(label, kind);
...
class SymfonyCompletionProvider {
    ...
    async serviceItems() {
        const services = await console_1.symfonyConsole.getServices();
        return Object.entries(services).map(([id, def]) => {
            const item = new SymfonyCompletionItem(id, vscode.CompletionItemKind.Class, () => `**Service ID:** \`${id}\`\n\n**Class:** \`${def.class ?? 'n/a'}\`\n\n**Public:** ${def.public ? 'yes' : 'no'}`);
    ...
    async routeItems() {
        const routes = await console_1.symfonyConsole.getRoutes();
        return Object.entries(routes).map(([name, def]) => {
            const item = new SymfonyCompletionItem(name, vscode.CompletionItemKind.Reference, () => `**Route:** \`${name}\`\n\n**Path:** \`${def.path ?? 'n/a'}\`\n\n**Method:** ${def.method ?? 'ANY'}\n\n**Controller:** \`${def.controller ?? 'n/a'}\``);
            item.detail = def.path ?? '';
            item.sortText = name;
            return item;
        });
    }
    async parameterItems() {
        ...
        return Object.entries(params).map(([name, value]) => {
            ...
            const item = new SymfonyCompletionItem(name, vscode.CompletionItemKind.Variable, () => `**Parameter:** \`${name}\`\n\n**Value:** \`${preview}\``);
            item.detail = preview;
            item.sortText = name;
            return item;
        });
