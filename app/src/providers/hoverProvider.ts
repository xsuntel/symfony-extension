...
export class SymfonyHoverProvider implements vscode.HoverProvider {
    /**
     * Token matching runs first and synchronously, so a line with nothing Symfony-ish on it
    ...
    provideHover(
        ...
        if (isYaml) {
            ...
            if (service) {
                return this.serviceHover(service, token);
            }
            const param = tokenAt(document, position, YAML_PARAM);
            if (param) {
                return this.paramHover(param, token);
            }
            return null;
        }
        ...
        if (route) {
            return this.routeHover(route, token);
        }
    ...
    private async serviceHover(id: string, token: vscode.CancellationToken): Promise<vscode.Hover | null> {
        const def = (await symfonyConsole.getServices())[id];
        if (!def || token.isCancellationRequested) {
            return null;
        }
    ...
    private async routeHover(name: string, token: vscode.CancellationToken): Promise<vscode.Hover | null> {
        const def = (await symfonyConsole.getRoutes())[name];
        if (!def || token.isCancellationRequested) {
            return null;
        }
    ...
    private async paramHover(name: string, token: vscode.CancellationToken): Promise<vscode.Hover | null> {
        const params = await symfonyConsole.getParameters();
        if (!(name in params) || token.isCancellationRequested) {
            return null;
        }
