...
export class SymfonyDefinitionProvider implements vscode.DefinitionProvider {
    async provideDefinition(
        ...
        const def = (await symfonyConsole.getServices())[serviceId];
        if (!def?.class || token.isCancellationRequested) {
            return null;
        }

        const uri = await this.findClassFile(def.class, token);
     ...
     */
    private async findClassFile(fqcn: string, token: vscode.CancellationToken): Promise<vscode.Uri | null> {
        ...
        const files = await vscode.workspace.findFiles(pattern, '**/vendor/**', 5, token);
        if (files.length === 0) {
            return null;
        }
