...
└── app/                                     ← Symfony Application Root
    └── src/
        ├── symfony/
        │   └── console.js                   ← Singleton: runs bin/console, caches results (30s TTL)
        ├── providers/
        │   ├── completionProvider.js        ← CompletionItemProvider (PHP + YAML)
        │   ├── hoverProvider.js             ← HoverProvider (PHP + YAML)
        │   └── definitionProvider.js        ← DefinitionProvider for service classes (PHP + YAML)
        └── views/
            ├── servicesTreeProvider.js      ← TreeDataProvider: Symfony services
            ├── routesTreeProvider.js        ← TreeDataProvider: Symfony routes
            └── parametersTreeProvider.js    ← TreeDataProvider: Symfony parameters
```
